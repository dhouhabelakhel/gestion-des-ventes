...
def affiche(code):
    ...
    for client in clients:
        if client['code']==code:
            nom_label=Label(app,text="Nom").grid(row=2,column=1)
            nom=Label(app,text=client['nom']).grid(row=2,column=2)
            prenom_label=Label(app,text="Prenom").grid(row=3,column=1)
            prenom=Label(app,text=client['prenom']).grid(row=3,column=2)
            adresse_label=Label(app,text="adresse").grid(row=4,column=1)
            adresse=Label(app,text=client['adresse']).grid(row=4,column=2)
            for commande in client['commandes']:
                num_cmd_label=Label(app,text='commande n° ').grid(row=command_row,column=1)
...
def show_article(event,articles_select):
    ...
    article_qte=tk.Spinbox(app, from_=0, to=200)
    article_qte.grid(row=row,column=6)
    add_button=Button(app,text="add_article",command=lambda:add_articles(article,article_qte.get())).grid(row=row,column=7)
    row=row+1
...
def command_total_form():
    ...
    num_cmd_entry=Entry(app,textvariable=num_cmd_value).grid(row=1,column=2)
    confirm_button=Button(text="montant ",padx=5,pady=5 ,command=lambda:total_cmd(num_cmd_value.get())).grid(row=1,column=3)
def client_command_form():
    clear_interface()
    code_label=Label(app,text="Code").grid(row=1,column=1)
    code_value=StringVar()
